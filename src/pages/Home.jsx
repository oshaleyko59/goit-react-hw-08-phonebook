...
export default function Home() {
  ...
  useEffect(() => {
    if (firstView === true) { dispatch(setFirstView(false)) };
  }, [dispatch, firstView]);
  ...
  return (
    <SkeletonText isLoaded={!isRefreshingUser && isContactsFetched}>
      {shoulNavigate ? (<Navigate to="/contacts" />)
      : (<Flex
        direction="column"
        spacing="20px"
        px="32px"
        ...
        <SkeletonText isLoaded={!isRefreshingUser}>
          ...
          <Box w={[300, 400, 500]}>
            <Text fontSize="24px" color={Colors.blue}>
              Here you can store your contacts and retrieve them whenever you
              need to.
            </Text>
          </Box>
          <Text my="16px" fontSize="20px" color={Colors.blue}>
            Please {user ? 'go to Contacts page' : 'login or register'} to enjoy
            the service.
          </Text>
        </SkeletonText>
